...
import organics_ml.utils as utils
...
class rnnCell(nn.Module):
    ...
    def dt_tauy(self):
        return rnnCell.tau_func(self.param_dt_tauy, self.dt_tau_max_y, self.beta)

    def dt_taua(self):
        return rnnCell.tau_func(self.param_dt_taua, self.dt_tau_max_a, self.beta)
    
    def dt_taub0(self):
        return rnnCell.tau_func(self.param_dt_taub0, self.dt_tau_max_b, self.beta)
    
    def dt_taub1(self):
        return rnnCell.tau_func(self.param_dt_taub1, self.dt_tau_max_b, self.beta)
    ...
    def forward(self, x, y, a, b0, b1):
        ...
        a_new = a + self.dt_taua() * (
            -a
            + self.sigma ** 2 * b0 ** 2
            + F.linear(self.get_activation_y(y) * torch.relu(a), Way, bias=None)
        )
